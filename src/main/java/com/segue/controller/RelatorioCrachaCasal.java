...
@Named
@ViewScoped
public class RelatorioCrachaCasal implements Serializable {
	...
	public void emitir() {
		try {
			...
			ExecutorRelatorioDownload executor = new ExecutorRelatorioDownload("/jasper/"+nomeArquivo+"Casal.jasper",
					this.response, parametros, "CrachaCasal" + this.segueMe.getNumeroRomano().getNumeroRomano() + ".pdf");
	...
	/**
	 * Marca como impressos (cracha = false) todos os crachás de casal pendentes do
	 * Segue-Me selecionado. Deve ser acionado somente após confirmar que a
	 * impressão física saiu corretamente.
	 */
	public void marcarComoImpressos() {
		try {
			EventoCasalFilter filter = new EventoCasalFilter();
			filter.setCracha("SIM");
			filter.setSegueMe(segueMe);
			List<Long> ids = new ArrayList<>();
			for (Evento evento : eventoRepository.filtradosCasal(filter)) {
