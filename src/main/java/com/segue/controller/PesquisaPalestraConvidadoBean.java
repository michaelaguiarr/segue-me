...
@Named
@ViewScoped
public class PesquisaPalestraConvidadoBean implements Serializable {
	...
	@Inject
	private CadastroPalestraConvidadoService cadastroService;
	...
	private PalestraConvidadoFilter filter;
	...
	public PesquisaPalestraConvidadoBean() {
		evento = new Evento();
		filter = new PalestraConvidadoFilter();
	...
	public void inicializar() {
		...
		this.listaEventoSeguidor = repository.filtradosPalestraConvidado(this.filter);
	}
	...
	public void pesquisar() {
		listaEventoSeguidor = repository.filtradosPalestraConvidado(this.filter);
	...
	/**
	 * Gera o PDF de crachá apenas dos convidados selecionados (por id), sem alterar
	 * a flag de crachá.
	 */
	public void imprimirSelecionados() {
		...
		try {
			...
			ExecutorRelatorioDownload executor = new ExecutorRelatorioDownload(
					"/jasper/" + nomeArquivo + "Convidado.jasper", response, parametros, "CrachaSelecionados.pdf");
	...
	public void selecionar() throws IOException {
		...
		FacesContext.getCurrentInstance().getExternalContext().redirect(
				Constants.CONTEXT + "/evento-convidado/cadastro-evento-palestra.xhtml?convidado=" + evento.getId());
	}
	...
	public PalestraConvidadoFilter getFilter() {
		return filter;
	}

	public void setFilter(PalestraConvidadoFilter filter) {
		this.filter = filter;
	}
