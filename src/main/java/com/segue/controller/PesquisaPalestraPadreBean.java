...
@Named
@ViewScoped
public class PesquisaPalestraPadreBean implements Serializable {
	...
	@Inject
	private CadastroEventoPadreService cadastroService;
	...
	private PalestraPadreFilter filter;
	...
	public PesquisaPalestraPadreBean() {
		evento = new Evento();
		filter = new PalestraPadreFilter();
	...
	public void inicializar() {
		...
		this.listaEventoSeguidor = repository.filtradosPalestraPadre(filter);
	}
	...
	public void pesquisar() {
		listaEventoSeguidor = repository.filtradosPalestraPadre(this.filter);
	...
	/**
	 * Gera o PDF de crachá apenas dos padres selecionados (por id), sem alterar a
	 * flag de crachá.
	 */
	public void imprimirSelecionados() {
		...
		try {
			...
			ExecutorRelatorioDownload executor = new ExecutorRelatorioDownload(
					"/jasper/" + nomeArquivo + "Padre.jasper", response, parametros, "CrachaSelecionados.pdf");
	...
	public void selecionar() throws IOException {
		...
		FacesContext.getCurrentInstance().getExternalContext()
				.redirect(Constants.CONTEXT + "/evento-padre/cadastro-evento-palestra.xhtml?padre=" + evento.getId());
	}
	...
	public PalestraPadreFilter getFilter() {
		return filter;
	}

	public void setFilter(PalestraPadreFilter filter) {
		this.filter = filter;
	}
