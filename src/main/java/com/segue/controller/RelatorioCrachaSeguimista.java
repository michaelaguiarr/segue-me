...
public class RelatorioCrachaSeguimista implements Serializable {
	...
	public void emitir() {
		try {
			fotoService.materializarImagensSegueMe(this.segueMe);   // padroeira/retiro (crachá SF)
			fotoService.materializarImagensCirculo(this.segueMe);   // imagem de cada círculo
	...
	/**
	 * Marca como impressos (cracha = false) todos os crachás de seguimista
	...
	public void marcarComoImpressos() {
		try {
			...
			for (Evento evento : eventoRepository.filtradosSeguimista(filter)) {
