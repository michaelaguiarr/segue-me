...
@Named
@ViewScoped
public class RelatorioCrachaPadre implements Serializable {
	...
	public void emitir() {
		try {
			...
			ExecutorRelatorioDownload executor = new ExecutorRelatorioDownload("/jasper/"+nomeArquivo+"Padre.jasper",
	...
	/**
	 * Marca como impressos (cracha = false) todos os crachás de padre/bispo
	...
	public void marcarComoImpressos() {
		try {
			EventoPadreFilter filter = new EventoPadreFilter();
			filter.setCracha("SIM");
			filter.setSegueMe(segueMe);
			List<Long> ids = new ArrayList<>();
			for (Evento evento : eventoRepository.filtradosPadre(filter)) {
