...
@Named
@ViewScoped
public class RelatorioQuadrantePalestra implements Serializable {
	...
	public RelatorioQuadrantePalestra() {
		limpar();
	}
	...
	public void emitir() {
		try {
			String nomeArquivo = "";
			if (layout) {
				nomeArquivo = "quadrantePalestraLayout";
			} else {
				nomeArquivo = "quadrantePalestra";
			}
			...
			ExecutorRelatorioDownload executor = new ExecutorRelatorioDownload("/jasper/"+nomeArquivo+".jasper",
					this.response, parametros, "Palestrante" + this.segueMe.getNumeroRomano().getNumeroRomano() + ".pdf");
