...
@Named
@ViewScoped
public class RelatorioQuadranteSeguimista implements Serializable {
	...
	public RelatorioQuadranteSeguimista() {
		limpar();
	}
	...
	public void emitir() {
		try {
			String nomeArquivo = "";
			if (layout) {
				nomeArquivo = "quadranteSeguimistaLayout";
			} else {
				nomeArquivo = "quadranteSeguimista";
			}
			...
			ExecutorRelatorioDownload executor = new ExecutorRelatorioDownload("/jasper/" + nomeArquivo + ".jasper",
					this.response, parametros, "Quadrante" + this.segueMe.getNumeroRomano().getNumeroRomano() + "-"
							+ this.circulo.getCorCirculo().getDescricao() + ".pdf");
