...
public class CadastroPalestraSeguidorService implements Serializable {
	...
	public Evento salvar(Evento evento) throws NegocioException {
		Evento eventoJaExiste = repository.findBySeguidorEventoPalestra(evento.getSeguidor(), evento.getSegueMe(),
				evento.getPalestra());
		if (eventoJaExiste != null && !eventoJaExiste.equals(evento)) {
			throw new NegocioException("Seguidor já cadastrado na Palestra!");
		}
