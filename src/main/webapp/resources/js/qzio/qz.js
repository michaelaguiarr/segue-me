...
//}
/**
 * Deploy tray version of QZ, or Optionally used to deploy multiple versions of
 * the applet for mixed environments. Oracle uses document.write(), which puts
 * the applet at the top of the page, bumping all HTML content down.
 */
deployQZ();

function getCertificate(callback) {
	/*
	 * $.ajax({ method: 'GET', url: 'assets/auth/digital-certificate.txt',
	 * async: false, success: callback // Data returned from ajax call should be
	 * the site certificate });
	 */

	// Non-ajax method, only include public key and intermediate key
	callback("-----BEGIN CERTIFICATE-----\n"
			+ "MIIErjCCApigAwIBAgIBADALBgkqhkiG9w0BAQUwgZgxCzAJBgNVBAYTAlVTMQsw\n"
			...
			+ "JzAlBgkqhkiG9w0BCQEWGHN1cHBvcnRAcXppbmR1c3RyaWVzLmNvbTAeFw0xODAy\n"
			+ "MDIwNTAwMDBaFw0xOTAyMDMwNTAwMDBaMH0xCzAJBgNVBAYMAkJSMQswCQYDVQQI\n"
			+ "DAJSUjESMBAGA1UEBwwJQm9hIFZpc3RhMQswCQYDVQQKDAJDUzELMAkGA1UECwwC\n"
			+ "Q1MxETAPBgNVBAMMCGludGVybGFiMSAwHgYJKoZIhvcNAQkBDBFzZXJ2aWNlQGNz\n"
			+ "LmluZi5icjCCASAwCwYJKoZIhvcNAQEBA4IBDwAwggEKAoIBAQDOj1LWLQPeNRZ0\n"
			+ "QdCdTN6Tso1QjBER0B1cDwz/BfZyjAv1UQ/4zNIOgd+8Yn0rndyw+17senACpe3n\n"
			+ "RuY9owweKnqWRjXA7sE2e2I2pC8B17CJITqPuivwQ7x4tM4KdnU45moFO2A8z1Fj\n"
			+ "mkb0AZY86AbDzimI02H6w7Xn2aQobtk9U3cjTw1fWet0R+xp7JDVrFq4aAbFeaJx\n"
			+ "krfNTfrZ3g/cpmA4DTXHKI5Mt4siaxt0erSTWyas86NVCHdbkFkK39pMSV0/UQX0\n"
			+ "Com6pDf8/Bq6QCDgt8ugVVh8qfM6CLOb8eAbvghHaPuNQBqUm9Up6ElDpmQR3TAf\n"
			+ "V0iGUBLvAgMBAAGjIzAhMB8GA1UdIwQYMBaAFJCmULeE1LnqX/IFhBN4ReipdVRc\n"
			+ "MAsGCSqGSIb3DQEBBQOCAgEAfLsTYjy4Ow5xAgUrVoxNs+vIDbn89KSqBDcAzmzG\n"
			+ "2x6VcLsKZePcu8RzFpXIXy5eBDnjRzOCgq8Xtl9QnjA0uaB4Mir6GdFCcAE0FNDf\n"
			+ "3si5EZoUFbMgcHi3+5OaGTvoq22kluzKKaLUNwIZuIpxUHx8bsOtM9vZh6V8JjdY\n"
			+ "U6uByqgwCzdkylgNMqhmTNtPU3lAst0OoSJBTlDWUmoAAh0fkuDhJiJTSpIYQkbo\n"
			+ "rxMkpgoZiIuz0d1MwV9qMZojDjZLF5xN8/AiCGPUAKdLmgFU9C2wMRiRM/nfkIak\n"
			+ "Y7tIsV3LA1yCRawkao822FNknQOCwk8RAJkL4qKwNbiY7X6E5G6o43SqTEnSH4ss\n"
			+ "dxjpcUnLxDwaq2Jc9ULAHY25d3ShmfcsGhlVdVYoPKe2znUUzb4R6hIKY/43vUSV\n"
			+ "A47MKc2q0HFLShshJWqPlnJL3O3iisXZlr3c3mipoAcwQzVpS0aEaGLa9PeM0Jpz\n"
			+ "br/fQrEqv2uaLscZ7coOPkIosX84jyBp5ipS//oWHZqmZ2nNrgyQhPb4kuWehOdz\n"
			+ "GCueL+ChlB9fNmMiXI3PNvZvf12NwYw/1nzgbpGP8VdN0xYNV7sKkPyIb6mGhcfJ\n"
			+ "MlAIa0o9x5o2R6YAUewcL7vrAa3zoovHJymeeKrWPwjHVccvQ/bMrUuM5OCKL6n8\n"
			+ "/QE=\n"
			+ "-----END CERTIFICATE-----\n"
...
function signRequest(toSign, callback) {
	/*
	 * $.ajax({ method: 'GET', contentType: "text/plain", url:
	 * '/secure/url/for/sign-message.php?request=' + toSign, async: false,
	 * success: callback // Data returned from ajax call should be the signature
	 * });
	 */

	// Send unsigned messages to socket - users will then have to Allow/Deny
	// each print request
	callback();
}
...
/**
 * Returns whether or not the applet is not ready to print. Displays an alert if
 * not ready.
 */
function notReady() {
...
/*******************************************************************************
 * Prototype function for finding the "default printer" on the system Usage:
 * qz.findPrinter(); window['qzDoneFinding'] = function() {
 * alert(qz.getPrinter()); };
 ******************************************************************************/
...
/*******************************************************************************
 * Prototype function for finding the closest match to a printer name. Usage:
 * qz.findPrinter('zebra'); window['qzDoneFinding'] = function() {
 * alert(qz.getPrinter()); };
 ******************************************************************************/
...
/*******************************************************************************
 * Prototype function for listing all printers attached to the system Usage:
 * qz.findPrinter('\\{dummy_text\\}'); window['qzDoneFinding'] = function() {
 * alert(qz.getPrinters()); };
 ******************************************************************************/
...
/*******************************************************************************
 * Prototype function for printing raw EPL commands Usage:
 * qz.append('\nN\nA50,50,0,5,1,1,N,"Hello World!"\n'); qz.print();
 ******************************************************************************/
function printEPL2(code) {
	if (!qz.getPrinter()) {
		useDefaultPrinter();
	}
	qz.append(code);
...
function print2impressora(code, code2, print1, print2) {
	qz.findPrinter(print1);
	qz.append(code);
	qz.print();
	qzDonePrinting();

	qz.findPrinter(print2);
	qz.append(code2);
	qz.print();
	qzDonePrinting();
...
function printEPL() {
	...
	qz.append('A310,116,0,3,1,1,N,"FROM SAMPLE.HTML"\n');
	qz.append('A310,146,0,3,1,1,N,"QZINDUSTRIES.COM"\n');
	qz.appendImage(getPath() + 'assets/img/image_sample_bw.png', 'EPL', 150,
			300);
...
/*******************************************************************************
 * Prototype function for printing raw ESC/POS commands Usage:
 * qz.append('\n\n\nHello world!\n'); qz.print();
 ******************************************************************************/
function printESCP() {
	if (notReady()) {
	...
	qz.appendImage(getPath() + "assets/img/image_sample_bw.png", "ESCP",
			"single");
...
/*******************************************************************************
 * Prototype function for printing raw ZPL commands Usage:
...
function printZPL() {
	...
	// Hint: Carriage Return = \r, New Line = \n, Escape Double Quotes= \"
	qz.append('^XA\n');
	qz.append('^FO50,50^ADN,36,20^FDPRINTED USING QZ PRINT PLUGIN '
			+ qz.getVersion() + '\n');
...
/*******************************************************************************
 * Prototype function for printing syntactically proper raw commands directly to
 * a EPCL capable card printer, such as the Zebra P330i. Uses helper
 * appendEPCL() to add the proper NUL, data length, escape character and newline
 * per spec: https://km.zebra.com/kb/index?page=content&id=SO8390 Usage:
 * appendEPCL('A1'); qz.print();
 ******************************************************************************/
function printEPCL() {
	if (notReady()) {
		return;
...
/*******************************************************************************
 * Prototype function for printing raw base64 encoded commands Usage:
 * qz.append64('SGVsbG8gV29ybGQh'); qz.print();
 ******************************************************************************/
function print64() {
	if (notReady()) {
		return;
	...
	qz
			.append64('Ck4KcTYwOQpRMjAzLDI2CkI1LDI2LDAsMUEsMyw3LDE1MixCLCIxMjM0IgpBMzEwLDI2LDAsMywx'
...
/*******************************************************************************
 * Prototype function for controlling print spooling between pages Usage:
 * qz.setEndOfDocument('P1,1\r\n'); qz.setDocumentsPerSpool('5');
 * qz.appendFile('/path/to/file.txt'); window['qzDoneAppending'] = function() {
 * qz.print(); };
 ******************************************************************************/
function printPages() {
	if (notReady()) {
...
/*******************************************************************************
 * Prototype function for printing a single XML node containing base64 encoded
 * data. Usage: qz.appendXML('/path/to/file.xml'); window['qzDoneAppending'] =
 * function() { qz.print(); };
 ******************************************************************************/
function printXML() {
	if (notReady()) {
 ...
 * 
 * Usage: qz.appendHex('00AABBCCDDEEFF'); qz.appendHex('x00xAAxBBxCCxDDxEExFF');
 * qz.print();
 ******************************************************************************/
function printHex() {
	if (notReady()) {
...
/*******************************************************************************
 * Prototype function for printing a text or binary file containing raw print
 * commands. Usage: qz.appendFile('/path/to/file.txt');
 ...
 * Not to be used in combination with raw printers. Usage:
 * qz.appendImage('/path/to/image.png'); window['qzDoneAppending'] = function() {
 * qz.printPS(); };
 ******************************************************************************/
function printImage(scaleImage) {
	...
	// Optional, set up custom page size. These only work for PostScript
	// printing.
	// setPaperSize() must be called before setAutoSize(), setOrientation(),
	// etc.
	if (scaleImage) {
...
/*******************************************************************************
 * Prototype function for printing a PDF to a PostScript capable printer. Not to
 * be used in combination with raw printers. Usage:
 * qz.appendPDF('/path/to/sample.pdf'); window['qzDoneAppending'] = function() {
 * qz.printPS(); };
 ******************************************************************************/
function printPDF() {
	// if (notReady()) { /*return;*/ }
 ...
 * printer. Not to be used in combination with raw printers. Usage:
 * qz.appendHTML('
 * <h1>Hello world!</h1>
 * '); qz.printPS();
 ******************************************************************************/
function printHTML() {
	...
	// Append our image (only one image can be appended per print)
	qz.appendHTML('<html>' + '<table face="monospace" border="1px">'
			+ '<tr height="6cm">' + '<td valign="top">' + colA + '</td>'
			+ '<td valign="top">' + colB + '</td>' + '</tr>' + '</table>'
			+ '</html>');
 ...
 * Usage: qz.findNetworkInfo(); window['qzDoneFindingNetwork'] = function()
 * {alert(qz.getMac() + ',' + qz.getIP()); };
 ******************************************************************************/
function listNetworkInfo() {
	if (isLoaded()) {
		// Makes a quick connection to www.google.com to determine the active
		// interface
		// Note, if you don't wish to use google.com, you can customize the host
		// and port
		...
		window['qzDoneFindingNetwork'] = function() {
			alert("Primary adapter found: " + qz.getMac() + ", IP: "
					+ qz.getIP());
 ...
 * qz.setPaperSize("8.5in", "11.0in"); // US Letter qz.setAutoSize(true);
...
function printHTML5Page() {
	$("#qz-status").html2canvas({
		...
		onrendered : function() {
			...
			// Optional, set up custom page size. These only work for PostScript
			// printing.
			// setPaperSize() must be called before setAutoSize(),
			// setOrientation(), etc.
			qz.setPaperSize("8.5in", "11.0in"); // US Letter
			qz.setAutoSize(true);
			qz.appendImage($("canvas")[0].toDataURL('image/png'));
 ...
 * java console to expose potentially new applet features/enhancements. Warning,
 * this has been known to trigger some PC firewalls when it scans ports for
 * certain printer capabilities. Usage: (identical to appendImage(), but uses
 * html2canvas for png rendering) qz.setLogPostScriptFeatures(true);
 * qz.appendHTML("
 * <h1>Hello world!</h1>
 * "); qz.printPS();
 ******************************************************************************/
function logFeatures() {
	if (isLoaded()) {
		...
		alert('Logging of PostScript printer capabilities to console set to "'
				+ !logging + '"');
	}
}
 ...
 * printing rather than the Java-to-CUPS interface. This will write the raw
 * bytes to a temporary file, then execute a shell command. (i.e. lpr -o raw
 * temp_file). This was created specifically for OSX but may work on several
 * Linux versions as well. qz.useAlternatePrinting(true); qz.append('\n\nHello
 * World!\n\n'); qz.print();
 ******************************************************************************/
...
/*******************************************************************************
 * Prototype function to list all available com ports available to this PC used
 * for RS232 communication. Relies on jssc_qz.jar signed and in the /dist/
 * folder. qz.findPorts(); window['qzDoneFindingPorts'] = function() {
 * alert(qz.getPorts()); };
 ******************************************************************************/
 ...
 * communication. qz.openPort('COM1'); qz.openPort('/dev/ttyUSB0');
...
function openSerialPort() {
	if (isLoaded()) {
		...
		// Automatically called when "qz.openPort()" is finished (even if it
		// fails to open)
 ...
 * qz.closePort('COM1'); qz.closePort('/dev/ttyUSB0');
...
function closeSerialPort() {
	if (isLoaded()) {
		...
		// Automatically called when "qz.closePort() is finished (even if it
		// fails to close)
...
/*******************************************************************************
 * Prototype function to send data to the open port qz.setSerialBegin(chr(2));
 * qz.setSerialEnd(chr(13)); qz.setSerialProperties("9600", "7", "1", "even",
 * "none"); qz.send("COM1", "\nW\n");
 ******************************************************************************/
...
/**
 * Fixes some html formatting for printing. Only use on text, not on tags! Very
 * important! 1. HTML ignores white spaces, this fixes that 2. The right
 * quotation mark breaks PostScript print formatting 3. The hyphen/dash
 * autoflows and breaks formatting
 */
